...
public:
  ...
  private:
    ...
    ConstIterator(const NodeBuilder& builder, bool end = false)
      : iter_(end ? builder.nodes_.end() : builder.nodes_.begin())
    {
    }

    std::vector<NodeDescription>::const_iterator iter_;
  };
  ...
  NodeBuilder& editor(Editor_Type&& et)
  {
    ...
    nodes_.back().editor_ = make_editor(std::forward<Editor_Type>(et));
    return *this;
  }
