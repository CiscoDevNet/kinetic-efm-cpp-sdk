...
class Requester final
{
  ...
  /// @param ec The error code defines if this action was successful or not.
  using on_subscription_update =
    std::function<void(const NodePath& path, const SubscriptionUpdate& update, const std::error_code& ec)>;
...
public:
  ...
  void set(const NodePath& path, const Variant& value, PermissionLevel permission, on_set_response&& callback);
  ...
  void remove(const NodePath& path, on_remove_response&& callback);
