...
struct NodeInfo
{
  ...
  std::string enum_values_;         ///< Enum values as a comma separated list if NodeInfo::type_ is an enum
...
public:
  ...
  void register_callback(
    ...
    std::function<void(const NodePath&, CallbackOn)>&& responder_callback);
  ...
  void create_node_or_set_value(
    const NodeCreateOrSetValueDescription& desc,
    std::function<void(const NodePath& link_path, const std::error_code& ec)>&& callback);
