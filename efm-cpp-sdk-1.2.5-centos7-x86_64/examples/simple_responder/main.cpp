...
public:
  ...
  void deinitialize(const std::string& link_name, const std::error_code& ec)
  {
    if (!ec) {
      LOG_EFM_DEBUG(
        "SimpleResponderLink", cisco::efm_sdk::DebugLevel::l1, "Responder link '" << link_name << "' deinitialized");
    } else {
...
int main(int argc, char* argv[])
{
  ...
  std::unique_ptr<SimpleResponderLink> responder_link;
  try {
    responder_link.reset(new SimpleResponderLink(*link));
  } catch (const std::exception& ex) {
  ...
  link->set_on_initialized_handler(
    std::bind(&SimpleResponderLink::initialize, responder_link.get(), std::placeholders::_1, std::placeholders::_2));
  link->set_on_connected_handler(
    std::bind(&SimpleResponderLink::connected, responder_link.get(), std::placeholders::_1));
  link->set_on_disconnected_handler(
    std::bind(&SimpleResponderLink::disconnected, responder_link.get(), std::placeholders::_1));
  link->set_on_deinitialized_handler(
    std::bind(&SimpleResponderLink::deinitialize, responder_link.get(), std::placeholders::_1, std::placeholders::_2));
